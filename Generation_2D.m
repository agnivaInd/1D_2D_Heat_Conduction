...
for i = 1:(val^2)
    if i == 1
        R(i) = val_temp_bottom + val_temp_left + qdot*(delx^2)/k;
    end
    if (i>1)&&(i<val)
        R(i) = val_temp_bottom + qdot*(delx^2)/k;
    end    
    if i == val
        R(i) = val_temp_bottom + val_temp_right + qdot*(delx^2)/k;
    end
    if (rem(i,val)==0)&&(i~=val)
        R(i) = val_temp_right + qdot*(delx^2)/k;
    end
    if (rem(i,val)==1)&&(i~=1)
        R(i) = val_temp_left + qdot*(delx^2)/k;
    end    
    if i == val^2-val+1
        R(i) = val_temp_top + val_temp_left + qdot*(delx^2)/k;
    end
    if (i>(val^2-val+1))&&(i<val^2)
        R(i) = val_temp_top + qdot*(delx^2)/k;
    end    
    if i == val^2
        R(i) = val_temp_top + val_temp_right + qdot*(delx^2)/k;
    end
end
%disp(R);
