    ...
    for i = 1:l
        for j = 1:l
            ...
            elseif (j==l)&&((i~=1)||(i~=l))
                ...
                actual_Temp(i,j) = ((alpha*time_step)*(actual_Temp(i-1,j)+store_actual_Temp(i+1,j)+actual_Temp(i,j-1)+store_actual_Temp(i,j+1))/((delx)^2) + qdot*alpha*time_step/k + store_actual_Temp(i,j))/(1+4*alpha*time_step/(delx)^2);
            end
